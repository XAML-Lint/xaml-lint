# LX0502: Stepper Minimum is greater than Maximum
...
inverted range are the same as [LX0501](LX0501.md): the control parses, but it does not work.
The rule is narrowed to MAUI only because `<Stepper>` is not a standard XAML element on
other dialects.

As with LX0501, either value being a markup extension (`{Binding}`, `{StaticResource}`, …)
...
```xaml
<!-- xaml-lint disable LX0502 -->
<Stepper Minimum="10" Maximum="5" />
<Stepper Minimum="20" Maximum="15" />
<!-- xaml-lint restore LX0502 -->
```
