# LX0501: Slider Minimum is greater than Maximum

<!-- Upstream: RXT330. -->

**Applies to:** `Avalonia`  `MAUI`  `WPF`
...
WPF, .NET MAUI, and Avalonia accept the inconsistent pair at parse time and clamp at
runtime in framework-specific ways — the result is a control that looks fine in the
designer but behaves surprisingly at runtime. Avalonia specifically coerces `Maximum` up
to `Minimum` via `Math.Max`, so an author writing `Maximum="5" Minimum="10"` ends up with
both properties equal to 10 and silently loses the stated range. UWP/WinUI 3 raise a
runtime exception instead, so the rule excludes those dialects.
...
```xaml
<!-- xaml-lint disable LX0501 -->
<Slider Minimum="10" Maximum="5" />
<Slider Minimum="20" Maximum="15" />
<!-- xaml-lint restore LX0501 -->
```
