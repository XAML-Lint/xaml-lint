# LX0201: Prefer x:Bind over Binding
...
An attribute value uses the `{Binding …}` markup extension on a UWP, WinUI 3, or Uno
Platform XAML file. The rule is informational — both binding forms compile and run — but
the project's target dialect offers the compiled `{x:Bind …}` alternative.

## Rule description

On UWP, WinUI 3, and Uno Platform, `{x:Bind}` compiles to generated code that participates
in view-model type checking at build time and runs faster than runtime-reflected
`{Binding}`. The
...
nested converters. WPF and MAUI are excluded because they do not support `{x:Bind}`.
