...
public sealed class PrettyFormatterTest
{
    ...
    {
        var a = new Diagnostic("LX0100", Severity.Warning, "m1", "a.xaml", 1, 1, 1, 1, null);
        var b = new Diagnostic("LX0101", Severity.Warning, "m2", "a.xaml", 2, 1, 2, 1, null);
        var c = new Diagnostic("LX0200", Severity.Info, "m3", "b.xaml", 1, 1, 1, 1, null);
        ...
        output.Should().Contain("LX0100");
        output.Should().Contain("LX0200");
    }

    [Fact]
    public void Color_on_writes_escape_sequences()
    {
        var d = new Diagnostic("LX0001", Severity.Error, "x", "a.xaml", 1, 1, 1, 1, null);
        var sw = new StringWriter();
