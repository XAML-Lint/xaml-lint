...
| LX0601 | RXT320 | Line.Fill has no effect. Applies to all dialects — a `<Line>`'s geometry has zero interior area in WPF, WinUI 3, UWP, MAUI, Avalonia, and Uno, so `Fill` is a universal no-op. Presence of any `Fill` value on `<Line>` fires. Placed in the Usability category alongside superseded-API rules; category scope widened to cover "no runtime effect" markup. |
