...
public sealed class MsBuildFormatterTest
{
    ...
    {
        var d = new Diagnostic("LX0300", Severity.Warning, "x:Name 'myButton' should start with uppercase.", "src/Views/MainView.xaml", 12, 28, 12, 38, "https://help");
        var sw = new StringWriter();
        new MsBuildFormatter().Write(sw, new[] { d }, "0.1.0");

        var line = sw.ToString().TrimEnd();
        line.Should().Be("src/Views/MainView.xaml(12,28): warning LX0300: x:Name 'myButton' should start with uppercase. [https://help]");
    }

    [Fact]
    public void HelpUri_is_omitted_when_null()
    {
        var d = new Diagnostic("LX0001", Severity.Error, "bad", "f.xaml", 1, 1, 1, 1, null);
        var sw = new StringWriter();
        new MsBuildFormatter().Write(sw, new[] { d }, "0.1.0");

        sw.ToString().TrimEnd().Should().Be("f.xaml(1,1): error LX0001: bad");
    }
}
