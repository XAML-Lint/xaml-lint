...
public sealed class LX0700_ImageWithoutAccessibleDescriptionTest
{
    ...
    {
        XamlDiagnosticVerifier<LX0700_ImageWithoutAccessibleDescription>.Analyze(
            $"""
    ...
    {
        // AutomationId is the canonical test-automation hook on UI Automation and MAUI's
        // IElement; any value signals the author wired the image into automation, so screen
        // readers will also have something to announce via platform UIA/AccessibilityServices.
        XamlDiagnosticVerifier<LX0700_ImageWithoutAccessibleDescription>.Analyze(
            $"""
            <ContentPage xmlns="{MauiXmlns}">
